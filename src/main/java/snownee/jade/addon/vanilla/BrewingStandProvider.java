...
public class BrewingStandProvider implements IComponentProvider, IServerDataProvider<TileEntity> {
    ...
    public void appendBody(List<ITextComponent> tooltip, IDataAccessor accessor, IPluginConfig config) {
        ...
        RenderableTextComponent stack = Renderables.item(new ItemStack(Items.BLAZE_POWDER));
        RenderableTextComponent fuelText = Renderables.offsetText(new TranslationTextComponent("jade.brewingStand.fuel", fuel), 0, 4);
        if (time > 0) {
