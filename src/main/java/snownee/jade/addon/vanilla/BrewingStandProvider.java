...
public class BrewingStandProvider implements IComponentProvider, IServerDataProvider<TileEntity> {
	...
	public void appendTooltip(ITooltip tooltip, BlockAccessor accessor, IPluginConfig config) {
		...
		tooltip.append(helper.text(new TranslationTextComponent("jade.brewingStand.fuel", TextFormatting.WHITE.toString() + fuel)).translate(new Vector2f(0, 2))); //TODO cache vector
		if (time > 0) {
			tooltip.append(helper.spacer(5, 0));
			tooltip.append(helper.item(new ItemStack(Items.CLOCK), 0.75f));
			tooltip.append(helper.text(new TranslationTextComponent("jade.brewingStand.time", TextFormatting.WHITE.toString() + time / 20)).translate(new Vector2f(0, 2)));
		}
	}
